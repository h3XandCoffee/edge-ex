...
const SalePercent: FC<{ loading: boolean; intl: any }> = ({ loading, intl }) => {
...
//const intl = useIntl();
  ...
  return (
    <Card
      ...
      title={intl.formatMessage({ id: 'app.dashboard.salePercent.proportionOfSales' })}
      loading={loading}
      extra={
        <Radio.Group value={dataType} onChange={e => setDataType(e.target.value)} buttonStyle="solid">
          <Radio.Button value="all">{intl.formatMessage({ id: 'app.dashboard.salePercent.all' })}</Radio.Button>
          <Radio.Button value="online">{intl.formatMessage({ id: 'app.dashboard.salePercent.online' })}</Radio.Button>
          <Radio.Button value="offline">{intl.formatMessage({ id: 'app.dashboard.salePercent.offline' })}</Radio.Button>
        </Radio.Group>
      }
    >
      <Row gutter={20}>
            ...
            renderItem={(item, index) => {
              ...
              return (
                <List.Item>
                  ...
                  <span>{item.name[intl.locale]}</span> | <span>{item.value}</span> <span>¥ {percent}</span>
                </List.Item>
              );
            }}
          />
...
export default injectIntl(SalePercent);
